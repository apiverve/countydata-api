...
class CountydataValidationException implements Exception {
  final List<String> errors;

  CountydataValidationException(this.errors);

  @override
  String toString() => 'CountydataValidationException: ${errors.join("; ")}';
}
...
/// County Data Lookup API client.
///
/// For more information, visit: https://apiverve.com/marketplace/countydata?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [state] (required) - The two letter name of the US state the county is in (e.g., MO) [minLength: 2, maxLength: 2]
/// * [county] (required) - The name of the US county to get data about (e.g. Jackson)
class CountydataClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'state': ValidationRule(type: 'string', required: true, minLength: 2, maxLength: 2),
    'county': ValidationRule(type: 'string', required: true),
  };

  CountydataClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/countydata',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CountydataValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CountydataValidationException(errors);
    }
  }

  /// Execute a request to the County Data Lookup API.
  ///
  /// Parameters are validated before sending the request.
  Future<CountydataResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CountydataException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CountydataResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CountydataException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CountydataException('Resource not found');
      } else {
        throw CountydataException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CountydataException) rethrow;
      throw CountydataException('Request failed: $e');
    }
  }
...
/// Exception thrown by the County Data Lookup API client.
class CountydataException implements Exception {
  final String message;

  CountydataException(this.message);

  @override
  String toString() => 'CountydataException: $message';
}
